...
public class Swerve {
	...
	public Swerve() {
		frontLeft = new SwerveDriveModule(Ports.FRONT_LEFT_ROTATION,
				Ports.FRONT_LEFT_DRIVE, 2);
		frontRight = new SwerveDriveModule(Ports.FRONT_RIGHT_ROTATION,
				Ports.FRONT_RIGHT_DRIVE, 1);
		rearLeft = new SwerveDriveModule(Ports.REAR_LEFT_ROTATION,
				Ports.REAR_LEFT_DRIVE, 3);
		rearRight = new SwerveDriveModule(Ports.REAR_RIGHT_ROTATION,
				Ports.REAR_RIGHT_DRIVE, 4);
	...
	public void sendInput(double x, double y, double rotateX, double rotateY, boolean halfPower, boolean robotCentric, boolean moonManeuver) {
		...
		if (moonManeuver) {
			...
			if ((rotateX > 0.1 || rotateX < -0.1)) {
				headingController = HeadingController.Off;
				rotateInput = rotateX;
			} else if (rotateX > -0.1 && rotateX < 0.1
					&& headingController == HeadingController.Off) {
			...
			if (headingController == HeadingController.On) {
				...
				if (angleDiff < 5) {
					rotationCorrection = (_targetAngle - currentAngle)
							* kPgainSmall - (currentAngularRate) * kDgainSmall;
					rotationCorrection = Cap(rotationCorrection,
							kMaxCorrectionRatioSmall);
				} else {
					rotationCorrection = (_targetAngle - currentAngle) * kPgain
							- (currentAngularRate) * kDgain;
					rotationCorrection = Cap(rotationCorrection,
							kMaxCorrectionRatio);
				}
				SmartDashboard.putNumber("ROTATE_CORRECT", rotationCorrection);
			} else if (headingController == HeadingController.Off) {rotationCorrection = 0;}
			if (halfPower) {
				y = y * 0.5;
				x = x * 0.5;
			} else {
			...
			if (x == 0.0 && y == 0.0) {rotateInput = rotateInput + rotationCorrection;}
			else {
				if (halfPower) {rotateInput = (rotateInput * rotationScaleFactor) + rotationCorrection;}
				else {rotateInput = (rotateInput * rotationScaleFactorFast) + rotationCorrection;}
			}
			if (robotCentric) {
				xInput = x;
				yInput = y;
	...
	public class SwerveDriveModule {
		...
		public void updateCoord() {
			double distanceTravelled = ((leftDriveEncoder.getDistance() + rightDriveEncoder.getDistance()) / 2.0) - distanceLast; // distanceLast doesn't appear to exist
			double timePassed = System.currentTimeMillis() - timeLast; // ditto for timeLast; apparently, I'm terrible at copying and pasting :(
		...
		public void debugValues() {
			// Note #3
			SmartDashboard.putNumber("ROT_" + Integer.toString(moduleID),
					Util.boundAngle0to360Degrees(getCurrentAngle()));
			SmartDashboard.putNumber("DRV_" + Integer.toString(moduleID),
					driveMotor.get());
		}
